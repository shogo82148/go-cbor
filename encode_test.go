...
func TestMarshal(t *testing.T) {
	tests := []struct {
		...
		{
			"byte string: h'01020304': Go no-addressable array",
			[...]byte{0x01, 0x02, 0x03, 0x04},
			[]byte{0x44, 0x01, 0x02, 0x03, 0x04},
		},
		{
			"byte string: h'01020304': Go addressable array",
		...
		{
			"Bigfloat 0.1",
			newBigFloat("0.1"), // ~ 0xcccccccccccccccdp-67
			[]byte{
				0xc5,       // Tag 5
				0x82,       // Array 2
				0x38, 0x42, // -67
				0x1b, 0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0xcd, // 0xcccccccccccccccd
			},
		},
		{
			"Bigfloat 100.1",
			newBigFloat("100.1"), // ~ 0x6419999999999999p-56
			[]byte{
				0xc5,       // Tag 5
				0x82,       // Array 2
				0x38, 0x38, // -47
				0x1b, 0xc8, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, // 0xc833333333333333
			},
		},
		...
		// known tag types
		{
			...
			ExpectedBase64URL{Content: []byte{0x01, 0x02, 0x03, 0x04}},
			[]byte{0xd5, 0x44, 0x01, 0x02, 0x03, 0x04},
		},
		{
			"expected base64 encoding",
			ExpectedBase64{Content: []byte{0x01, 0x02, 0x03, 0x04}},
			[]byte{0xd6, 0x44, 0x01, 0x02, 0x03, 0x04},
		},
		{
			"expected base16 encoding",
			ExpectedBase16{Content: []byte{0x01, 0x02, 0x03, 0x04}},
			[]byte{0xd7, 0x44, 0x01, 0x02, 0x03, 0x04},
		},
		{
			"encoded data",
			EncodedData{0x64, 0x49, 0x45, 0x54, 0x46},
			[]byte{0xd8, 0x18, 0x45, 0x64, 0x49, 0x45, 0x54, 0x46},
		},
		{
			"uri",
